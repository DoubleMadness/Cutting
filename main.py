...
def cutting():
    ...
    try:
        ...
        match action:
            case "1":
                cut_frame(file, 1, "normal")
                file.close()
            case "2":
                cut_clip(file)
                file.close()
            case "3":
                user_amount = input("Write amount of Frames - ")
                cut_frame(file, int(user_amount), "random")
                file.close()
    except:
        error()
        cutting()
...
def cut_frame(file, amount, _type):
    if _type not in ["random", "normal"]:
        error()
        cutting()
    ...
    if _type == "normal":
        ...
        max_time = input("Write the end time (Example: X:XX:XX) - ")
        max_time = calculator(max_time)
        clear()

        for step in range(1, amount+1):
            name = "Pictures/Random_Frame" + get_time() + ".png"
            time = random.randint(0, max_time)
            output_frame = file.get_frame(int(time))
            output = Image.fromarray(output_frame)
            output.save(name)
            print("Frame " + str(step) + " - complete!")
            sleep(0.01)
